...
const bottomFooterStyle: React.CSSProperties = { flex: '0 0 auto', paddingTop: '8px', borderTop: '1px solid rgba(0,0,0,0.1)' };
...
export const LayoutFrame: React.FC<LayoutFrameProps> = ({
  ...
  return (
    <div style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column', background: '#111' }} data-role="layout-frame" data-overlay={isOverlay ? 'true' : 'false'}>
      <header style={{ flex: '0 0 auto', padding: '12px 16px', background: 'var(--header-bg)', color: 'white' }} data-role="header">{header}</header>

      {/* Main semantic region as content panel */}
      <main style={{ flex: '1 1 auto', minHeight: 0, display: 'flex', flexDirection: 'column', background: 'var(--main-bg)' }} data-role="content-wrapper" aria-label="Content Panel">
        <section style={{ flex: '1 1 auto', minHeight: 0, overflowY: 'auto', padding: '16px', boxSizing: 'border-box', color: 'white', position: 'relative', paddingBottom: contentExtraPadding ? `${contentExtraPadding}px` : undefined, display: 'flex', flexDirection: 'column' }} data-role="content-panel" data-content-overlay-pad={contentExtraPadding ? contentExtraPadding : 0}>
          {contentPanel}
        </section>
      </main>

      {hasComposerPanel && !isOverlay && (
        <section ref={(el) => (bottomRef.current = el)} style={{ ...inlineComposerStyle, background: 'var(--composer-bg)', color: '#111', padding: '12px 16px', boxSizing: 'border-box' }} data-role="bottom-region" data-mode="inline" aria-label="Composer Region">
      ...
      {hasComposerPanel && isOverlay && (
        <section ref={(el) => (bottomRef.current = el)} style={{ position: 'fixed', left: 0, right: 0, bottom: 0, zIndex: 20, background: 'var(--composer-bg)', color: '#111', padding: '12px 16px', boxSizing: 'border-box', ...overlayComposerStyle }} data-role="bottom-region" data-mode="overlay" aria-label="Composer Region (Overlay)">
          <div style={bottomRegionInnerStyle} data-role="bottom-region-inner">
            <div style={composerScrollAreaStyle} data-role="composer-panel" aria-label="Composer Panel">{composerPanel}</div>
            {hasFooter && <div style={bottomFooterStyle} data-role="footer">{footer}</div>}
          </div>
      ...
      {!hasComposerPanel && hasFooter && (
        <footer style={{ flex: '0 0 auto', padding: '8px 16px', background: 'var(--footer-bg)', color: 'white' }} data-role="footer" data-footer-standalone="true">{footer}</footer>
      )}
    </div>
  );
};
